...
# stop2 = np.array([-84.53, -14.84, -9.72, -80.10, 0.73, -6.11, 16.36])
# stop2 = deg2rad(stop2)
# stop3 = stop2
# stop2 = np.concatenate((stop2, gripper_init), 0).tolist()


# close_state = {
#       'gripper_joint':0.68,
#       'l_finger_passive_joint':0.68,
#       'r_finger_1_joint':-0.67989,
#       'r_finger_passive_joint':-0.67989,
#       'l_finger_2_joint':-0.67993,
#       'r_finger_2_joint':0.67996  
# }
...
# joint_names = gym.get_actor_dof_names(env, robot_handle)
# # print(joint_names)
# close_jt = []
# for joints in joint_names[7:]:
#       close_jt.append(open_state[joints])




# upper = np.array(robot_props["upper"][7:])
# lower = np.array(robot_props["lower"][7:])
...
# stop3 = np.array(dof_pos)
# clost_jt = np.array(close_jt)
# stop3 = np.concatenate((stop3, close_jt), 0).tolist()
# print(nominal)
# nominal[0] = 0.
# gym.set_actor_dof_states(env, robot_handle, stop3, gymapi.STATE_POS)
# gym.set_actor_dof_position_targets(env, robot_handle, inital_pos_rad)
# t = 0
# gym.set_actor_dof_velocity_targets(env, robot_handle, stop3)
# stop3 = np.array(dof_pos)
# stop3 = np.concatenate((stop3, ))
nprinted = True

M = gym.acquire_mass_matrix_tensor(sim, "robot")
M = gymtorch.wrap_tensor(M)
print(M)
